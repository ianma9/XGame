    ...
    public class ControllerBase : ApiController
    {
        ...
        public async Task<HttpResponseMessage> ResponseAsync(object result, IServiceBase serviceBase)
        {
            ...
            if (!serviceBase.Notifications.Any())
            {
                ...
                {
                    return Request.CreateResponse(HttpStatusCode.Conflict, $"There was an internal problem with the server. Contact your System Administrator if the problem persists. Internal error: {ex.Message}");
                }
            }
            else
            {
